...
USE_INCLUDE_ENDPOINTS = os.environ.get("USE_INCLUDE_ENDPOINTS") is not None
...
def isect_segments(s):
    ret = sweepline.isect_segments(s, USE_INCLUDE_ENDPOINTS)
    return tuple(sorted(set(ret)))


def isect_segments__naive(s):
    ret = isect_naive.isect_segmens_naive(s, USE_INCLUDE_ENDPOINTS)
    return tuple(sorted(set(ret)))
